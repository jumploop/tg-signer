...
前后端分离架构：后端为 FastAPI（提供 REST API 并托管前端静态产物），前端为 Vue 3（源码位于 `webui_frontend/`）。正常使用无需自行构建前端，`pip install "tg-signer[gui]"` 安装的包内已含构建产物；如需修改前端，在 `webui_frontend/` 下执行 `npm install && npm run build`，产物会输出到 `tg_signer/webui/static/`。
